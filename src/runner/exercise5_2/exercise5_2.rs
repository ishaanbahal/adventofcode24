...
impl ModRunner for Worker {
    fn run(&self) {
        let safe_count = parse_input("runner/exercise5_2/testfile");
...
fn validate_sequences(sequences: Vec<Vec<i32>>, ruleset: HashMap<i32, Vec<i32>>) -> i32 {
    ...
    for seq in sequences {
        ...
        if !is_valid_seq {
            let fixed_seq = reorder_seq(&seq, &ruleset);
            let seq_mid_point: usize = (seq.len() - 1) / 2;
            sequence_mid_total += fixed_seq[seq_mid_point];
        }
    }
    sequence_mid_total
}

fn reorder_seq(sequenece: &Vec<i32>, ruleset: &HashMap<i32, Vec<i32>>) -> Vec<i32> {
    let mut seq = sequenece.clone();
    for page_index in 0..seq.len() {
        if page_index == seq.len() - 1 {
            break;
        }
        let mut iterator = page_index;
        loop {
            let current_page = seq[page_index];
            let next_page = seq[iterator];
            let next_page_rules = ruleset.get(&next_page);
            match next_page_rules {
                None => {
                    // No rule found
                }
                Some(rule) => {
                    if rule.contains(&current_page) {
                        // swap the pages
                        let temp = seq[page_index];
                        seq[page_index] = seq[iterator];
                        seq[iterator] = temp;
                    }
                }
            }
